...
const REFERRAL_STORAGE_KEY = "tecai:pending-ref";
...
function parseReferrerFromPayoutDetails(value?: string) {
  const raw = String(value || "");
  const match = raw.match(/referrer:([^|]+)/i);
  return match?.[1] ? normalizeEmail(match[1]) : "";
}

function parseDepositTxIdFromPayoutDetails(value?: string) {
  const raw = String(value || "");
  const match = raw.match(/deposit_tx:([^|]+)/i);
  return match?.[1] ? String(match[1]).trim() : "";
}

function isReferralBonusMethod(method?: string) {
  const normalized = String(method || "").trim().toUpperCase();
  return (
    normalized === "REFERRAL_BONUS_5" ||
    normalized === "REFERRAL 5% DEPOSIT BONUS" ||
    normalized === "REFERRAL_5_PERCENT"
  );
}

function isReferralBonusTx(tx: Tx) {
  const payoutDetails = String(tx.payoutDetails || "");
  return (
    tx.type === "deposit" &&
    tx.status === "approved" &&
    (isReferralBonusMethod(tx.method) || payoutDetails.includes("bonus_type:referral_5") || payoutDetails.includes("deposit_tx:"))
  );
}

function isReferralBonusNotification(notif: AppNotif) {
...
function extractBonusAmount(text: string) {
...
export default function App() {
  ...
  const [pendingReferral, setPendingReferral] = useState(() => {
    try {
      return normalizeEmail(localStorage.getItem(REFERRAL_STORAGE_KEY) || "");
    } catch {
      return "";
    }
  });
  ...
  const [currentUserReferrer, setCurrentUserReferrer] = useState("");
  ...
  const [adminActionTxId, setAdminActionTxId] = useState("");
  ...
  const referralBonusTotal = useMemo(() => {
    const txBonusTotal = txs.filter(isReferralBonusTx).reduce((sum, tx) => sum + tx.amount, 0);
    if (txBonusTotal > 0) return txBonusTotal;

    const notifBonusTotal = notifications
      .filter(isReferralBonusNotification)
      .reduce((sum, notif) => sum + extractBonusAmount(notif.message), 0);
    return notifBonusTotal;
  }, [txs, notifications]);
  ...
  useEffect(() => {
    const refFromUrl = parseReferralFromLocation();
    if (!refFromUrl || refFromUrl === ADMIN_EMAIL) return;
    setPendingReferral((prev) => prev || refFromUrl);
  }, []);
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem(REFERRAL_STORAGE_KEY, pendingReferral);
    } catch {
  ...
  async function ensureUserRow(emailValue: string, referredBy?: string) {
    const normalizedEmail = normalizeEmail(emailValue);
    const normalizedRef = normalizeEmail(referredBy || "");
    const existingUser = await getUserByEmail(normalizedEmail);
    if (!existingUser) {
      const created = await createUser(normalizedEmail, "local_user", normalizedRef || undefined);
      if (!created) {
        return false;
      }
    }

    if (normalizedRef && normalizedRef !== normalizedEmail && supabase) {
      await supabase
        .from("users")
  ...
  async function loadInvitedUsers(emailValue: string) {
    ...
    const direct = await supabase
      ...
      .ilike("referred_by", normalizedOwner);
  ...
  async function loadUserData(emailValue: string) {
    ...
    const fallbackRef = normalizeEmail(parseReferralFromLocation() || pendingReferral);
    if (!existingUser) {
      const referredBy = fallbackRef && fallbackRef !== normalizeEmail(emailValue) ? fallbackRef : undefined;
      await ensureUserRow(emailValue, referredBy);
      setCurrentUserReferrer(referredBy || "");
    } else {
      const existingRef = normalizeEmail(String((existingUser as UserRow).referred_by || ""));
      const finalRef = existingRef || (fallbackRef && fallbackRef !== normalizeEmail(emailValue) ? fallbackRef : "");
      if (finalRef && !existingRef) {
        await ensureUserRow(emailValue, finalRef);
      }
      setCurrentUserReferrer(finalRef);
    }
    ...
    await loadInvitedUsers(emailValue);
  }

  useEffect(() => {
    if (!session || session.role !== "user" || tab !== "team") return;
    void loadInvitedUsers(session.email);
    void getTransactions(session.email).then((rows) => {
      setTxs((rows || []).map(mapTx));
    });
  }, [session, tab]);

  async function loadAdminData(withUsers = false) {
    if (adminLoadInFlightRef.current) return;
    adminLoadInFlightRef.current = true;
    setIsAdminRefreshing(true);
    try {
      const pendingRows = await getTransactions(undefined, "pending");
      setTxs((pendingRows || []).map(mapTx));
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const liveDevices = (await getDevices(currentEmail)).map(mapDevice);
      const now = Date.now();
      let totalAdded = 0;
  ...
  }, [session, currentEmail, lang]);

  async function reconcileReferralBonusesForUser(ownerEmail: string) {
    if (!isSupabaseConfigured || !supabase) return;
    const normalizedOwner = normalizeEmail(ownerEmail);
    if (!normalizedOwner || normalizedOwner === ADMIN_EMAIL) return;
    ...
    const invitedEmails = (invited.data || [])
      .map((row: { email?: string }) => normalizeEmail(String(row.email || "")))
      .filter(Boolean);
    ...
    const { data: approvedDeposits } = await supabase
      .from("transactions")
      .select("id,email,amount,status,type")
      .in("email", invitedEmails)
      .eq("type", "deposit")
      .eq("status", "approved");

    const myApprovedDeposits = (await getTransactions(normalizedOwner, "approved", "deposit")).map(mapTx);
    const rewardedDepositIds = new Set(
      myApprovedDeposits
        .filter(isReferralBonusTx)
        .map((tx) => parseDepositTxIdFromPayoutDetails(tx.payoutDetails))
        .filter(Boolean),
    );

    const missingDeposits = (approvedDeposits || []).filter((row: { id: string }) => !rewardedDepositIds.has(String(row.id)));
    if (missingDeposits.length === 0) return;

    let totalBonus = 0;
    for (const deposit of missingDeposits) {
      const baseAmount = Number((deposit as { amount?: number }).amount || 0);
      const bonusAmount = Number((baseAmount * REFERRAL_DEPOSIT_RATE).toFixed(4));
      ...
      const created = await createTransaction({
        ...
        type: "deposit",
        amount: bonusAmount,
        method: "REFERRAL_BONUS_5",
        status: "approved",
        payout_details: `bonus_type:referral_5|from:${String((deposit as { email?: string }).email || "")}|deposit_tx:${String((deposit as { id?: string }).id || "")}`,
      ...
      await createNotification({
        email: normalizedOwner,
        title: "مبروك لقد تحصلت على مكافئة الإحالة",
        message: `مبروك لقد تحصلت على مكافئة الإحالة بقيمة ${bonusAmount.toFixed(2)} ${t.tnd}`,
      });
    }

    if (totalBonus <= 0) return;

    const currentBal = await getBalance(normalizedOwner);
    await updateBalance(
      normalizedOwner,
      Number((Number(currentBal?.balance || 0) + totalBonus).toFixed(4)),
      Number(currentBal?.total_deposit || 0),
    );
  }

  useEffect(() => {
    if (!session || session.role !== "user") return;
    void reconcileReferralBonusesForUser(session.email);
    const timer = window.setInterval(() => {
      void reconcileReferralBonusesForUser(session.email);
    }, 180000);
    return () => window.clearInterval(timer);
  ...
  useEffect(() => {
    if (!session || session.role !== "user" || tab !== "team") return;
    void reconcileReferralBonusesForUser(session.email).then(() => {
      void loadUserData(session.email);
    });
  }, [session, tab]);
  ...
  async function handleAuthSubmit() {
    ...
    try {
      ...
      if (authMode === "register") {
        ...
        if (signIn.error && !String(signIn.error.message || "").toLowerCase().includes("email not confirmed")) {
          return setNotice(signIn.error.message);
        }

        const referredByRaw = parseReferralFromLocation() || pendingReferral;
        const referredBy = referredByRaw && referredByRaw !== safeEmail ? referredByRaw : undefined;
        const userReady = await ensureUserRow(safeEmail, referredBy);
        if (!userReady) {
          setNotice(lang === "ar" ? "تم إنشاء الحساب. جاري مزامنة بيانات المستخدم..." : "Account created. Syncing user profile...");
        }
        await updateBalance(safeEmail, 0, 0);
      ...
      if (safeEmail === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
        ...
        if (adminSignIn.error) return setNotice(t.badCredentials);
      ...
      if (!existing) {
        const fallbackRef = parseReferralFromLocation() || pendingReferral;
        const ok = await ensureUserRow(safeEmail, fallbackRef || undefined);
        if (!ok) {
          setNotice(lang === "ar" ? "تم تسجيل الدخول. جاري مزامنة بيانات المستخدم..." : "Logged in. Syncing user profile...");
        }
        await updateBalance(safeEmail, 0, 0);
      } else {
        const existingRef = normalizeEmail(String((existing as UserRow).referred_by || ""));
        const fallbackRef = parseReferralFromLocation() || pendingReferral;
        if (fallbackRef && !existingRef && fallbackRef !== safeEmail) {
          await ensureUserRow(safeEmail, fallbackRef);
        }
      }
  ...
  async function handleDeposit() {
    ...
    const fallbackRef = normalizeEmail(parseReferralFromLocation() || pendingReferral);
    const effectiveReferrer = normalizeEmail(currentUserReferrer || fallbackRef);
    ...
    const created = await createTransaction({
      ...
      method: tx.method,
      status: tx.status,
      payout_details: effectiveReferrer && effectiveReferrer !== currentEmail ? `referrer:${effectiveReferrer}` : undefined,
      ...
      if (!created) {
        setNotice(lang === "ar" ? "تعذر إرسال طلب الإيداع" : "Failed to submit deposit request");
        return;
      }
  ...
  async function handleWithdraw() {
    ...
    const created = await createTransaction({
      ...
      if (!created) {
        setNotice(lang === "ar" ? "تعذر إرسال طلب السحب" : "Failed to submit withdrawal request");
        return;
      }
  ...
  async function handleBuy(plan: (typeof PRODUCTS)[number]) {
    ...
    if (!ok) {
      setActionDialog({
        ...
        message:
          lang === "ar"
            ? "لم يتم حفظ الجهاز في قاعدة البيانات. تحقق من صلاحيات RLS لجدول devices ثم أعد المحاولة."
            : "Device was not saved to database. Check RLS insert policy on devices table and try again.",
      });
      return;
    }
    ...
    if (!balanceOk) {
      setActionDialog({
        ...
        message:
          lang === "ar"
            ? "تم إنشاء الجهاز لكن تعذر خصم السعر من الرصيد. تحقق من صلاحيات balances ثم أعد المحاولة."
            : "Device was created but balance deduction failed. Check balances policies and try again.",
      });
      await loadUserData(currentEmail);
      return;
    }
  ...
  async function applyReferralBonusIfNeeded(tx: Tx) {
    if (tx.type !== "deposit" || tx.status !== "approved") return;

    const referrerFromTx = parseReferrerFromPayoutDetails(tx.payoutDetails);
    let referrerEmail = referrerFromTx;
    if (!referrerEmail) {
      const depositorUser = await getUserByEmail(tx.email);
      referrerEmail = normalizeEmail(String((depositorUser as UserRow | null)?.referred_by || ""));
    }

    if (!referrerEmail || referrerEmail === normalizeEmail(tx.email)) return;

    const refTxs = (await getTransactions(referrerEmail, "approved", "deposit")).map(mapTx);
    const bonusExists = refTxs.some(
      (refTx) => isReferralBonusTx(refTx) && String(refTx.payoutDetails || "").includes(tx.id)
    );
    if (bonusExists) return;

    const bonusAmount = Number((tx.amount * REFERRAL_DEPOSIT_RATE).toFixed(4));
    if (bonusAmount <= 0) return;

    const inviterBalance = await getBalance(referrerEmail);
    const nextInviterBalance = Number((Number(inviterBalance?.balance || 0) + bonusAmount).toFixed(4));
    const updated = await updateBalance(
      referrerEmail,
      nextInviterBalance,
      Number(inviterBalance?.total_deposit || 0)
    );
    if (!updated) return;
    ...
    await createTransaction({
      ...
      method: "REFERRAL_BONUS_5",
      status: "approved",
      payout_details: `bonus_type:referral_5|from:${tx.email}|deposit_tx:${tx.id}`,
      created_at: new Date().toISOString(),
    });
  ...
  async function handleApproveReject(tx: Tx, status: "approved" | "rejected") {
    if (adminActionTxId) return;
    setAdminActionTxId(tx.id);
    try {
    if (tx.type === "deposit" && status === "approved" && !tx.receiptDataUrl) {
      setNotice("لا يمكن قبول الإيداع بدون إثبات دفع");
      return;
    }
    const result = await rpcAdminDecideTransaction(tx.id, status);
    if (!result) {
      setNotice("فشل تحديث العملية");
      return;
    }

    const resultObj = typeof result === "object" && result !== null ? (result as { ok?: boolean; message?: string }) : null;
    if (resultObj && resultObj.ok === false) {
      setNotice(String(resultObj.message || "تعذر تنفيذ العملية"));
      return;
    }

    if (status === "approved") {
      await applyReferralBonusIfNeeded({ ...tx, status: "approved" });
    }
  ...
  async function handleSendNotification() {
    ...
    try {
      const ok = await rpcAdminSendNotification({ email: normalizeEmail(notifUser), title: notifTitle, message: notifBody });
      if (!ok) {
        setActionDialog({
          open: true,
          ...
          message:
            lang === "ar"
              ? "تعذر حفظ الإشعار الآن. تحقق من صلاحيات جدول notifications ثم أعد المحاولة."
              : "Could not save notification now. Check notifications permissions and try again.",
        });
        return;
      }
      setNotifUser("");
        ...
        <div className="relative w-full max-w-md rounded-3xl border border-zinc-200 bg-white/95 p-6 shadow-lg backdrop-blur-sm">
          ...
          <div className="mt-6 space-y-3">
            ...
            <button
              ...
              className="w-full rounded-xl bg-blue-600 py-3 font-bold text-white disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isAuthSubmitting ? (lang === "ar" ? "يرجى الانتظار..." : "Please wait...") : authMode === "login" ? t.login : t.register}
            </button>
            <button
              onClick={() => setAuthMode((p) => (p === "login" ? "register" : "login"))}
              disabled={isAuthSubmitting}
              className="w-full rounded-xl border border-zinc-300 py-3 font-bold disabled:cursor-not-allowed disabled:opacity-60"
            >
              {authMode === "login" ? t.register : t.login}
            </button>
          </div>
          {isBusy && <p className="mt-3 rounded-xl bg-amber-50 p-2 text-center text-xs text-amber-700">{lang === "ar" ? "جاري تنفيذ العملية... يرجى الانتظار" : "Processing... please wait"}</p>}
    ...
    return (
      <div className="min-h-screen bg-zinc-100 p-4" dir={lang === "ar" ? "rtl" : "ltr"}>
        <div className="mx-auto max-w-3xl space-y-4">
          ...
          {isAdminRefreshing && <p className="rounded-xl bg-amber-50 p-2 text-center text-xs text-amber-700">{lang === "ar" ? "جاري التحديث... يرجى الانتظار" : "Updating... please wait"}</p>}
              ...
              <div key={tx.id} className="rounded-xl border border-zinc-200 bg-white p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <button
                    ...
                    disabled={(tx.type === "deposit" && !tx.receiptDataUrl) || Boolean(adminActionTxId)}
                    className="rounded-lg bg-emerald-600 py-2 text-white disabled:cursor-not-allowed disabled:bg-emerald-300"
                  >
                    {adminActionTxId === tx.id ? (lang === "ar" ? "جاري التنفيذ..." : "Working...") : t.approve}
                  </button>
                  <button
                    onClick={() => void handleApproveReject(tx, "rejected")}
                    disabled={Boolean(adminActionTxId)}
                    className="rounded-lg bg-rose-600 py-2 text-white disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {adminActionTxId === tx.id ? (lang === "ar" ? "جاري التنفيذ..." : "Working...") : t.reject}
                  </button>
                </div>
              </div>
            ))
          ...
          <div className="rounded-xl border border-zinc-200 bg-white p-4">
            ...
            <input value={notifUser} onChange={(e) => setNotifUser(normalizeEmail(e.target.value))} placeholder="user@email.com" className="mb-2 w-full rounded-lg border border-zinc-300 px-3 py-2" />
            ...
            <button
              ...
              className="w-full rounded-lg bg-indigo-600 py-2 text-white disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isNotificationSending ? (lang === "ar" ? "جاري الإرسال..." : "Sending...") : t.sendNotification}
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-100 pb-20" dir={lang === "ar" ? "rtl" : "ltr"}>
      <main className="mx-auto max-w-4xl p-4">
        ...
        {isBusy && <p className="mb-3 rounded-xl bg-amber-50 p-2 text-center text-xs text-amber-700">{lang === "ar" ? "جاري تنفيذ العملية... يرجى الانتظار" : "Processing... please wait"}</p>}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="font-semibold">
                          {isReferralBonusNotification(n)
                            ? (lang === "ar" ? "مبروك لقد تحصلت على مكافئة الإحالة" : "Referral bonus")
                            : n.title}
                        </p>
                        <p className="text-sm text-zinc-600">
                          {isReferralBonusNotification(n)
                            ? (lang === "ar"
                              ? `مبروك لقد تحصلت على مكافئة الإحالة بقيمة ${extractBonusAmount(n.message).toFixed(2)} ${t.tnd}`
                              : `Congrats, you received a referral bonus of ${extractBonusAmount(n.message).toFixed(2)} ${t.tnd}.`)
                            : n.message}
                        </p>
        ...
        {route === "home" && tab === "home" && (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <button className="w-full overflow-hidden rounded-2xl border border-zinc-300 bg-white px-4 py-3 text-center text-sm font-semibold md:text-base" dir="ltr" title={currentEmail || "+21624400500"}>
                <span className="block truncate">{currentEmail || "+21624400500"}</span>
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="min-h-[132px] rounded-2xl border border-blue-300 bg-gradient-to-br from-blue-600 to-indigo-600 p-4 text-white shadow-sm">
                <p
                  dir="ltr"
                  className="font-extrabold leading-tight tracking-tight text-[clamp(1.15rem,5.2vw,1.75rem)]"
            ...
            <div className="relative overflow-hidden rounded-3xl border border-zinc-200 bg-white">
              <img
                src={HOME_SLIDER_IMAGES[homeSliderIndex]}
                alt="Robots slider"
          ...
          <div className="space-y-4 rounded-xl border border-zinc-200 bg-white p-4">
            ...
            {depositDestination && (
              <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm">
                ...
                {(depositMethod === "D17" || depositMethod === "Flouci") ? (
                  <p className="font-mono text-2xl font-bold tracking-wide text-blue-900">55810112</p>
                ) : (
                  <p className="break-all font-mono text-blue-900">{depositDestination}</p>
                )}
                <button
                  onClick={() => void navigator.clipboard.writeText((depositMethod === "D17" || depositMethod === "Flouci") ? "55810112" : depositDestination)}
            ...
            <button
              ...
              className="w-full rounded-lg bg-blue-600 py-2.5 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isDepositSubmitting ? (lang === "ar" ? "جاري إرسال الطلب..." : "Submitting...") : t.submitDeposit}
            </button>
          </div>
        )}
        ...
        {route === "withdraw" && (
          <div className="space-y-4 rounded-xl border border-zinc-200 bg-white p-4">
            ...
            <button
              ...
              className="w-full rounded-lg bg-zinc-900 py-2.5 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isWithdrawSubmitting ? (lang === "ar" ? "جاري إرسال الطلب..." : "Submitting...") : t.submitWithdraw}
            </button>
          </div>
        )}
          ...
          <div className="space-y-3">
            {devices.length === 0 ? (
              ...
              devices.map((d) => {
                ...
                const elapsedMs = Math.max(0, now - purchased);
                const runHours = Math.floor(elapsedMs / (60 * 60 * 1000));
                const runMinutes = Math.floor((elapsedMs % (60 * 60 * 1000)) / (60 * 1000));
                const isFinished = now >= end || d.earnedAmount >= d.totalIncome;
                const liveProfit = Math.min(d.totalIncome, Number(((progress / 100) * d.totalIncome).toFixed(4)));
                ...
                return (
                  <div key={d.id} className="rounded-xl border border-zinc-200 bg-white p-3">
                    ...
                    <img src={d.image || PRODUCT_IMAGE_MAP[d.planCode] || PRODUCT_IMAGE_MAP.A1} alt={d.planCode} className="mb-2 h-28 w-full rounded-lg object-cover" />
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <p>السعر: {formatAmount(d.planPrice)} {t.tnd}</p>
                      <p>الربح اليومي: {formatAmount(d.dailyIncome)} {t.tnd}</p>
                      <p>الربح المباشر: {formatAmount(liveProfit)} {t.tnd}</p>
                      <p>الأرباح المحصلة: {formatAmount(d.earnedAmount)} {t.tnd}</p>
                      <p>الأرباح المتبقية: {formatAmount(remaining)} {t.tnd}</p>
                      <p>مدة التشغيل: {runHours}h {runMinutes}m</p>
                    </div>
        ...
        {route === "home" && tab === "team" && (
          <div className="space-y-3">
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-4">
              ...
              <p className="mb-3 rounded-lg bg-blue-50 p-2 text-sm text-blue-700">
                {lang === "ar"
                  ? `عدد أعضاء الفريق: ${invitedUsers.length} | مجموع مكافآت الإحالة 5%: ${formatAmount(referralBonusTotal)} ${t.tnd}`
                  : `Team members: ${invitedUsers.length} | Total 5% referral bonuses: ${formatAmount(referralBonusTotal)} ${t.tnd}`}
              </p>
