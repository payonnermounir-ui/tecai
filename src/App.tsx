...
async function updateUserBalance(phone: string, newBalance: number): Promise<boolean> {
  ...
  const { error } = await supabase
    .from('wallets')
    .upsert({ phone: phone, balance: newBalance }, { onConflict: 'phone' });
  ...
  useEffect(() => {
    if (session?.role === "user" && session.phone && supabase) {
      ...
      fetchUserBalance(session.phone).then(remoteBalance => {
        if (remoteBalance !== balances[session.phone]) {
  ...
  async function onUserAuthSubmit() {
    ...
    if (authMode === "register") {
      ...
      if (isSupabaseConfigured && supabase) {
        ...
        // إنشاء ملف شخصي ومحفظة للمستخدم الجديد
        await supabase.from('profiles').upsert({ phone: fullPhone, login_password: password });
        await supabase.from('wallets').upsert({ phone: fullPhone, balance: 0 });
      }
