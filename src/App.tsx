...
export default function App() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      for (const device of devices) {
        const last = new Date(device.lastPayoutAt).getTime();
      ...
      if (totalAdded > 0) {
        const nextBalance = Number((balance + totalAdded).toFixed(4));
        await updateBalance(currentEmail, nextBalance);
  ...
  async function handleAuthSubmit() {
    ...
    // Admin login: try Supabase session first, then fallback to local admin mode.
    if (safeEmail === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
      ...
      // If admin account is missing in Auth, create it once and sign in.
      ...
      // Keep admin access available even when Supabase Auth admin session fails.
  ...
  async function handleBuy(plan: (typeof PRODUCTS)[number]) {
    ...
    if (!ok) {
      setNotice("فشل حفظ الجهاز");
      return;
    }

    const nextBalance = Number((balance - plan.price).toFixed(4));
    await updateBalance(currentEmail, nextBalance);
    await loadUserData(currentEmail);
    setActionDialog({
      open: true,
  ...
  async function handleApproveReject(tx: Tx, status: "approved" | "rejected") {
    ...
    // Always reload from DB so pending badge and admin list stay consistent.
    await loadAdminData();
  ...
  async function handleSendNotification() {
    ...
    await createNotification({ email: notifUser, title: notifTitle, message: notifBody });
    setNotifUser("");
    setNotifTitle("");
    setNotifBody("");
    setNotice(t.sent);
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="min-h-[132px] rounded-2xl border border-blue-300 bg-gradient-to-br from-blue-600 to-indigo-600 p-4 text-white shadow-sm">
                <p dir="ltr" className="text-[22px] font-extrabold leading-tight tracking-tight md:text-[24px]">
                  <span className="inline-block w-full text-center">{formatStatAmount(totalDeposit)}</span>
                </p>
                <p className="mt-2 text-white/90">{t.totalDeposit}</p>
              </div>
              <div className="min-h-[132px] rounded-2xl border border-blue-300 bg-gradient-to-br from-blue-600 to-indigo-600 p-4 text-white shadow-sm">
                <p dir="ltr" className="text-[22px] font-extrabold leading-tight tracking-tight md:text-[24px]">
                  <span className="inline-block w-full text-center">+{formatStatAmount(todayProfit)}</span>
                </p>
                <p className="mt-2 text-white/90">{t.todayProfit}</p>
              </div>
              <div className="min-h-[132px] rounded-2xl border border-blue-300 bg-gradient-to-br from-blue-600 to-indigo-600 p-4 text-white shadow-sm">
                <p dir="ltr" className="text-[22px] font-extrabold leading-tight tracking-tight md:text-[24px]">
                  <span className="inline-block w-full text-center">{formatStatAmount(balance)}</span>
                </p>
