...
export default function App() {
  ...
  const referralBonusTotal = useMemo(
    () =>
      txs
        .filter((tx) => tx.status === "approved" && tx.method === "REFERRAL_BONUS_5")
...
setInvitedUsers(rows);
  ...
  async function applyReferralBonusIfNeeded(tx: Tx) {
    ...
    const bonusExists = refTxs.some(
      (refTx) => refTx.method === "REFERRAL_BONUS_5" && String(refTx.payoutDetails || "").includes(tx.id)
    );
    ...
    await createTransaction({
      ...
      status: "approved",
      payout_details: `from:${tx.email}|deposit_tx:${tx.id}`,
      created_at: new Date().toISOString(),
    });

    await createNotification({
      email: referrerEmail,
      title: lang === "ar" ? "مكافأة إحالة" : "Referral bonus",
      message:
        lang === "ar"
          ? `تم إضافة مكافأة إحالة 5% بقيمة ${bonusAmount.toFixed(2)} ${t.tnd} بعد إيداع صديقك.`
          : `A 5% referral bonus of ${bonusAmount.toFixed(2)} ${t.tnd} was added after your friend's deposit.`,
    });
  }
