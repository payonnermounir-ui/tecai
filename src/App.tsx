...
export default function App() {
  ...
  async function loadUserData(emailValue: string) {
    ...
    if (!existingUser) {
      ...
      await createUser(emailValue, "local_user", referredBy);
      setCurrentUserReferrer(referredBy || "");
    } else {
      setCurrentUserReferrer(normalizeEmail(String((existingUser as UserRow).referred_by || "")));
    }
  ...
  async function handleAuthSubmit() {
    ...
    if (authMode === "register") {
      ...
      await createUser(safeEmail, password, referredBy);
      if (referredBy && supabase) {
        // Ensure referred_by is persisted even if the insert path hit an existing user row.
        await supabase
          .from("users")
    ...
    if (safeEmail === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
      ...
      if (adminSignIn.error) {
        setNotice("تم فتح لوحة الأدمن بوضع محلي");
      }
    ...
    if (!existing) {
      ...
      if (fallbackRef && !existingRef && fallbackRef !== safeEmail && supabase) {
        await supabase
          .from("users")
          .update({ referred_by: fallbackRef })
          .eq("email", safeEmail)
          .is("referred_by", null);
  ...
  async function handleSendNotification() {
    ...
    const ok = await createNotification({ email: normalizeEmail(notifUser), title: notifTitle, message: notifBody });
    if (!ok) {
      setActionDialog({
        open: true,
