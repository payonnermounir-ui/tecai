...
export default function App() {
  ...
  const referralBonusTotal = useMemo(() => txs.filter(isReferralBonusTx).reduce((sum, tx) => sum + tx.amount, 0), [txs]);
  ...
  }, [session, devices, currentEmail, lang]);
  ...
  async function handleDeposit() {
    ...
    await createTransaction({
      ...
      method: tx.method,
      status: tx.status,
      payout_details: currentUserReferrer ? `referrer:${currentUserReferrer}` : undefined,
  ...
  async function applyReferralBonusIfNeeded(tx: Tx) {
    ...
    await createNotification({
      email: referrerEmail,
      title: lang === "ar" ? "مبروك لقد تحصلت على مكافئة الإحالة" : "Referral bonus received",
      message:
        lang === "ar"
          ? `مبروك لقد تحصلت على مكافئة الإحالة بقيمة ${bonusAmount.toFixed(2)} ${t.tnd}`
          : `Congrats, you received a referral bonus of ${bonusAmount.toFixed(2)} ${t.tnd}.`,
    });
  }
