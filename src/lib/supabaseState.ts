...
export async function createUser(email: string, passwordHash: string, referredBy?: string) {
  ...
  if (referredBy) {
    // نستخدم الإيميل مباشرة للبحث عن المحيل لأن الرابط يستخدم الإيميل
    ...
    if (refUser?.email) {
      // إعطاء مكافأة تسجيل (اختياري، حالياً 5)
      await addBalance(refUser.email, 5);

      // 🔥 تسجيل الإحالة في جدول referrals ليظهر في صفحة الفريق فوراً
      await supabase.from('referrals').insert({
        user_email: email,
        referrer_email: refUser.email,
        bonus: 0 // المكافأة الحقيقية تأتي عند الإيداع أو مكافأة تسجيل ثابتة
      });
    }
  }
...
export async function createTransaction(tx: any) {
  ...
  // ==================== REFERRAL SYSTEM (FIXED DEPOSIT BONUS) ====================
  if (tx.type === 'deposit' && tx.status === 'approved') {
    ...
    if (user?.referred_by) {
      ...
      // 🔥 FIX: البحث عن المحيل بالإيميل (لأن referred_by يخزن الإيميل)
      ...
      // تسجيل أو تحديث سجل الإحالة بمكافأة الإيداع
      await supabase.from('referrals').insert({
        user_email: tx.email,
...
export async function updateTransactionStatus(id: string, status: string) {
  ...
  if (status === 'approved' && tx.type === 'deposit') {
    ...
    // 🔥 تشغيل منطق الإحالة عند الموافقة على الإيداع
    await createTransaction({ ...tx, status: 'approved' });
  }
