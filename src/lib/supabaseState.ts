...
export async function createUser(email: string, passwordHash: string, referredBy?: string) {
  ...
  try {
    ...
    if (error) {
      ...
      if (!fallbackWithoutHash.error) return { email, referred_by: referredBy ?? null };

      // Fallback for schemas that do not include referred_by.
      const fallback = await supabase
        .from("users")
        .insert({ email, password_hash: passwordHash });
      if (!fallback.error) return { email, referred_by: null };
      ...
      if (fallbackMinimal.error) {
        // Duplicate means row already exists and this is fine.
        if (fallbackMinimal.error.code === "23505") return { email, referred_by: referredBy ?? null };
        return null;
      }
      return { email, referred_by: referredBy ?? null };
    }
    return { email, referred_by: referredBy ?? null };
  } catch {
    return null;
...
export async function createBalance(email: string) {
  ...
  try {
    await supabase
      .from("balances")
      .upsert({ email, balance: 0, total_deposit: 0, updated_at: new Date().toISOString() }, { onConflict: "email" });
    return { balance: 0, total_deposit: 0 };
  } catch {
    return null;
  }
...
export async function updateBalance(email: string, balance: number, totalDeposit?: number) {
  if (!isSupabaseConfigured || !supabase) return null;
  try {
    const updates: { email: string; balance: number; updated_at: string; total_deposit?: number } = {
      email,
      balance,
    ...
    if (error) return null;
    return { email, balance, total_deposit: totalDeposit };
  } catch {
    return null;
  }
}
...
export async function createProfit(profit: {
  ...
  if (!isSupabaseConfigured || !supabase) return null;
  try {
    const { error } = await supabase.from("profits").insert(profit);
    if (error) return null;
    return profit;
  } catch {
    return null;
